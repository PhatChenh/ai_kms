...
| C4 | knowledge_entries upsert lacks UNIQUE constraint | ✅ Confirmed real — INTENTIONAL | Migration 008 has no UNIQUE index on `(dimension, entity, tag, fact)`. This is explicitly documented as deferred to Phase 8 (roadmap note: "Natural-key dedup is deferred to Phase 8 (DQ-2 in the plan)"). **Phase 8 must address this.** |
...
| MEDIUM | C4 — knowledge_entries UNIQUE constraint | Duplicate entries possible | Phase 8 must add this (already documented as DQ-2) |
