...
Use when the feature introduces new modules, classes, or services and
the human needs to see what talks to what.

```
┌─────────────┐     ┌──────────────┐     ┌─────────────┐
│  Watcher     │────▶│  Pipeline    │────▶│  Storage    │
│  (fs events) │     │  (extract →  │     │  (SQLite)   │
└─────────────┘     │   classify)  │     └─────────────┘
                     └──────┬───────┘
                            │ confidence < 0.60
                            ▼
...
with existing systems) → 2–3 diagrams of different types.
