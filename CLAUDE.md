...
### Phase 5 — cloud deployment (planned, not yet built) gotchas
- **Config env-override pattern is NOT one-size-fits-all — `KMS_DB_PATH`'s post-construction trick only works for fields with no construction-time validator.** `DatabaseConfig` carries `model_config = {"validate_assignment": True}` (`config.py:157`) and `database.path` has no existence gate, so `load_config()` assigns `cfg.main.database.path` AFTER `MainConfig` is built (`config.py:596-597`). `vault.root` is different: it is checked by a `MainConfig` `@model_validator(mode="after")` (`validate_vault_root_exists`, `config.py:372-382`) that fires AT CONSTRUCTION against the YAML value, and `VaultConfig` has NO `validate_assignment`. So copying the DB-path pattern for a `VAULT_ROOT` override crashes at import with "Vault root does not exist" before the assignment ever runs. Correct mechanism (P5 Slice 2 / TD-059): inject the env value into the raw config dict BEFORE `MainConfig(**raw_main)` — between `_load_yaml` (`config.py:582`) and the `Config(...)` call (`config.py:590-595`) — reading it via an `ApiKeys` `Field(alias="VAULT_ROOT")`. Also note `select_vault_by_env` (`config.py:356-370`) overrides root when `env == "test"`, so cloud env-files must set `env: prod`/`dev`. (Planning-derived; verified against code in P5 Slice 2 research, not yet built.)
