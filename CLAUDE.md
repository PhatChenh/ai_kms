...
### Phase 7.5 — daemon + capture gotchas
- **Every `asyncio.run_coroutine_threadsafe` call in DaemonLoop MUST attach `_log_future_exception` callback.** There are 4 sites (in `_on_move_window_expired`, `_on_create_or_modify`, `_on_move`, `_on_delete`). Without `.add_done_callback(self._log_future_exception)`, exceptions inside the fire-and-forget coroutine are silently swallowed — no log, no trace. If you add a new `run_coroutine_threadsafe` call inside DaemonLoop, you MUST attach the callback. Pattern: `fut = asyncio.run_coroutine_threadsafe(..., self._loop); fut.add_done_callback(self._log_future_exception)`.
- **`_best_effort_index` helper is shared between text and binary paths — body differs.** Text path passes `extracted_text` as body; binary path passes `summary or ""`. If you add a third call site, pick the right body for that context. The helper itself has no awareness of which path called it.
- **Auth header is set ONCE on `httpx.AsyncClient` constructor in `DaemonLoop.run()` and `scan_cmd`.** Uploader/event_reporter/scanner no longer pass per-request auth headers. If you add a new outbound HTTP call from the daemon, use the shared client — do NOT add a `headers={"Authorization": ...}` kwarg. The `status` command and `connection_check.py` are exceptions (separate short-lived clients).
