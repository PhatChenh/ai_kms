...
> **REARCHITECTURE IN PROGRESS (2026-06-12).** The system is being rearchitected for cloud-native deployment. Read `docs/0_draft/cloud_native_rearchitecture.md` as the authority for system direction. Content below describes the current local-only codebase — accurate for existing code, but stale for unimplemented phases. As you implement each rearchitecture piece, update the relevant sections in this file to match.
...
- **Every new migration breaks the previous migration's version-pin test.** `tests/test_storage/test_migration_<N>.py` asserts `schema_version == N` after `init_db`. Adding migration `N+1` bumps the version, so those assertions fail on a normal `pytest` run (no skip marker). When you add a migration, bump the prior version-pin assertions to the new number — this is the expected update, NOT a regression. (Hit during P5 Slice 1 planning: migration 008 → bump `test_migration_007.py:41,56` `7`→`8`.)
